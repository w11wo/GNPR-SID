...
def parse_args():
    ...
    parser.add_argument("--dataset_path", type=str, default="../LLM4POI/datasets/")
...
def main(args):
    ...
    data_path = Path(args.dataset_path) / "nyc" / "raw"
    ...
    val_df = pd.read_csv(data_path / "NYC_val.csv")
    test_df = pd.read_csv(data_path / "NYC_test.csv")
    df = pd.concat([train_df, val_df, test_df])

    # remap venue_category_id to schema.org's 162 categories
    df["Catname"] = df["POI_catid"].apply(lambda x: category2schema[x])

    df["Region"] = df.apply(lambda row: get_pluscode(row["latitude"], row["longitude"]), axis=1)
    df["Time"] = pd.to_datetime(df["local_time"], format="%Y-%m-%d %H:%M:%S+00:00").dt.hour

    df = df.rename(columns={"user_id": "Uid", "POI_id": "Pid"})
